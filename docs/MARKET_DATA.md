...
| `round_trip_status` | `checked` / `not checked` / `not checked: time budget spent` / `check failed: …`. ⛔ **Show "not checked" as not checked. Never render a missing verdict as a pass.** ⚠️ From 2026-09-19 a Jupiter HTTP failure or rate limit leaves `round_trip.verdict` **null** with the error in the status, instead of a false `NO_BUY_ROUTE` / `NO_SELL_ROUTE` |
| `safety`, `safety_reasons`, `safety_ts`, `safety_not_checked`, `safety_scope` | ⭐ **From 2026-09-19** (rows built by a pass after 07:20Z; earlier files have none). `DANGER` / `WARN` / `NO FLAGS` / `UNKNOWN` under `PRECOMMIT_safety_v1.md`, **S1 only**: built from this row's own `round_trip`, no extra call, so a row never round-tripped is `UNKNOWN`. `safety_scope` says "partial: S1 only…" - **show it**. `safety_not_checked` is never empty; ⛔ **NO FLAGS is never rendered without it, and never as "safe"**. Reasons are ordered most severe first. The full verdict (mint account, D1, holders, RugCheck) exists only for tracked tokens, in `data/universe/` (`safety_scope` "full: …"); untracked trending rows in `narratives.json` carry a "partial: mint account and D1 only" verdict. A missing `safety` field means **not computed**, never clean |

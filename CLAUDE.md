...
| figure | value | source |
|---|---|---|
| ⛔⛔ **graduation base rate, "published" - SUPERSEDED BY ITS OWN AUTHOR, verified 2026-09-23** | **0.198% [0.189, 0.208], n=832,941 is arXiv:2607.02823 v1 (2 Jul 2026).** ⛔ **v4, current since 10 Sep 2026, is a DIFFERENT PAPER**: retitled *"Auditing Collector-Generated Graduation Labels on Pump.fun: Measurement Error and Temporal Non-Generalization"*, **n=749,816**, and it **reports no pooled graduation rate at all.** Its findings: *"The collector's TIMEOUT label does not establish platform-side non-graduation"*, validation **AUROC 0.4642** against development 0.8594 with the interval **containing 0.5000**, and *"of nine automated evaluations, two pass, six fail, and one is not evaluable"*. ⛔⛔ **NEVER quote 0.198% or n=832,941 as current literature, and never in anything public.** ⛔ **Our own 0.22% therefore loses its corroboration and stands on our own measurement ALONE.** ⚠️ The Social-Presence results in `docs/CREATOR_PLAYBOOK.md` (1.919% vs 0.110%, Cox HR 4.506) are from the same superseded version. ⭐ **The author withdrew a survival analysis because its graduation labels came from an off-chain collector and could not be shown to measure the real event - the same defect class as our `gone` label, found independently in the literature** | `data/findings/CITATIONS_2026-09-23.md` |
| ⭐⭐ **the Marino citation, VERIFIED against arxiv.org 2026-09-23** | **arXiv:2602.14860 is real**: *"Predicting the success of new crypto-tokens: the Pump.fun case"*, **Marino, Naviglio, Tarantelli & Lillo, 16 Feb 2026**. ⭐ **n=655,770 verifies verbatim** (§IV: *"a total of 655,770 tokens were created by 243,123 distinct token creator addresses"*), and so does the result the whole product rests on — §VI: *"it is not possible to make profits with a buy-and-hold strategy based only on vSol"*, §VII: *"all conditional graduation-probability curves remain below the economic breakeven over most of the vSol range"*, eq. (3) is **p(vSol, θ) > vSol²/115²**. ⭐ **Nothing to change.** Also verified: MELT arXiv:2602.13480 (36.5%), *Catching the Rug* arXiv:2608.20271 (6.4M), Mongardini & Mei arXiv:2507.01963 — ⚠️ **the paper says 82.8%, not the 82.89% we quote** — and arXiv:2512.11850, ⛔ **whose author is DAVIDE MANCINO, not the "Mzoughi et al." we credit** | same |
...
| ⛔⛔ **DATA WAS DELETED 2026-09-23, and the brief was RIGHT: 29 launches, by design** | Confirmed from `data/funnel/2026-09.jsonl`. At **12:05Z (08:05 ET)** the scan stage recorded `skipped 429, carried_forward 400`, and **429 - 400 = 29** pools went on the floor with only their COUNT kept. ⛔ **Unrecoverable: the launch feed does not serve the past.** ⭐ **The run-up was visible for seven hours and nothing acted on it**: coverage 27.7% → 20.3% → 16.6% → 13.9% → 12.2% → 10.2% → 9.9% → 8.5% while the carry grew 107 → 400 and jammed at `CARRY_MAX`. Truncate reason every pass: *stage deadline, -5s to -9s left*, so the stage began already over budget. ⚠️ **The brief's other half is WRONG on its object**: the graduation backlog was **0 all day** (lag 0.01-0.12h), so there was no 400-graduation cap and no eight-hour stall there; what stalled was the SCAN stage, reaching ~40 of 469 pools. ⭐⭐ **FIXED: a backlog may DEGRADE, it may never DELETE.** The overflow now goes to an append-only spill (`data/_scan_carry_spill.jsonl`) drained back whenever the carry has room, cursor-advanced so ⭐ **a drained row is still on disk** (rule 8). `carry_dropped` is 0 by construction and `funnel.py` alarms at **spill depth > 0**, pre-committed before the first spilled row existed. `test_discipline.py` reads all 35 spilled rows **back off disk**, drains them, and proves every one returns (102/102). ⛔ **What this does NOT fix is throughput**: ~40 reached per 110s pass against ~57 arriving, so the debt grows by construction and the spill will grow with it. The spill makes it visible and recoverable, not solved | `scanner.py` carry block, `funnel.py` `SPILL_DEPTH_ALARM`, `data/funnel/2026-09.jsonl` |
...
| ⛔⛔ **"the pool no longer exists" has NEVER happened** | **0 of 5,816** distinct pools in 24h of outcome rows are closed on chain. `record_outcome` writes `gone` when two INDEXER lookups fail (`journal.py:974`), collapsing four states: closed / near-zero / indexer dropped it / wrong pool. **3,878 of 5,816 recorded pairs (67%) are pump.fun bonding curves**, and **243 rows on 135 contracts were scored against a curve AFTER our own ledger said the token graduated off it**. ⛔⛔ **RETRACTED AS A FINDING 2026-09-23: that re-ask was a TAUTOLOGY.** It read `getAccountInfo` on the pool address OUR OWN ROW recorded and **all 54 were `pumpfun_curve` accounts, which are never closed when a token bonds**, so "POOL_EXISTS 54, POOL_ABSENT 0" was guaranteed before the first RPC. **Superseded by the two rows above, which derive the pool from the MINT.** ⛔ **The new part: 6 of 60 `gone` rows recorded a pool address that NO AMM PROGRAM OWNS**, so for those rows we do not know which pool we priced. ⛔ **And existence is not liquidity** - vault balances were ruled out of scope on purpose, so this may never be quoted as "these tokens are alive" | same, BACKLOG A51-A52, A64, `PRECOMMIT_gone_onchain.md` |
| ⛔⛔ **POOL DISCOVERY FIXED: the pool is DERIVED from the mint, and the indexer is now only a SUPPLEMENT** | Frank: *"I would probably rather fix pool discovery. That's been a problem for us. We can't publish shit data."* Rule `PRECOMMIT_pool_state.md`. ⭐⭐ **The mechanism is now PROVEN, not inferred: all THREE `gone` contracts that turned out sellable had a recorded pair EQUAL TO THEIR DERIVED BONDING CURVE** (NOOS `4zhiY2Vk…`, JEANB `5Cozv7ka…`, LOOONGJAK `DyVDpYve…`). The curve still exists and holds **$0** while the PumpSwap pool holds **$397 / $439 / $361**. **We priced the abandoned curve and called the token dead.** ⛔ **CORRECTION to my own earlier report: I said 2 of 3 had a wrong recorded pair. It is 3 of 3** - run 1's flag measured whether the LADDER happened to also surface the curve, not whether the pair was right. ⭐ All three now resolve `pool_live` at the correct PumpSwap pool, which matches the address the ladder found independently | `PRECOMMIT_pool_state.md`, `poolstate.py` |
| ⛔⛔ **NO STRUCT OFFSET IS ASSUMED, and measuring them found a venue we did not have at all** | To find pools by `getProgramAccounts` you need a memcmp offset into someone else's account layout, and a guessed offset returns ZERO ROWS which reads as "no pool". So every offset is **MEASURED** by finding the mint's 32 raw bytes inside a pool already known to belong to it (`analysis/pool_offsets/probe.py` → `data/pools/offsets.json`, with the pool each offset came from). ⭐⭐ **It found `cpamdpZCGKUy5JxQXB4dcpGPiikHawvSWAd6mEn1sGG` holding 7 of 48 sampled pools with NO ENTRY IN OUR MAP: Meteora DAMM v2, which the indexer labels `DYN2`, and one of its pools is EMBER/MET.** ⭐ Measured offsets: pumpswap **[43, 75]**, meteora_dlmm [88,120], meteora_damm_v2 [168,200], raydium_clmm [73,105,454], orca_whirlpool [101,181,269], meteora_dynamic [8,40], raydium_v4 [400], ⭐ **meteora_dbc [136] and fluxbeam [99,131,163] added 2026-09-23** after measuring that **12 of 120 recorded pairs were DBC and 1 was FluxBeam and NEITHER venue had an offset, so neither was ever queried**; orca_whirlpool gained a 4th slot **397** and `unnamed_amm_9W959Dq` resolved to the 324-byte token-swap shape [99,131,163]. ⚠️ **Two venues are ONE-SIDED, carried as `venues_one_sided`: `meteora_dbc` stores one mint slot and its WSOL vault's mint is NOWHERE in its 424-byte struct (4/4 samples), and `raydium_v4` yields one because its vaults sit under a shared authority - a pool where our mint is on the other side is invisible to us.** ⭐ **`raydium_cpmm` now HAS a measured offset [168, 200] and is queried** - it was found by resolving every seed pair's owner on chain, having appeared in no seed mint's first 12 indexer pairs. It is PONDER's venue. ⛔ **PumpSwap's base slot 43 only exists because I added on-chain fixtures where OUR mint is the base** - the indexer returns 0 pairs for exactly these tokens, so seeding from SOL/USDC could only ever measure the QUOTE slot, and 25 of the 120 `gone` contracts are PumpSwap. ⭐⭐ **`raydium_launchlab` added 2026-09-24: 429-byte pool, base **205**, quote **237**, 4/4 samples** - and measuring it found that `AMM_OWNERS` carried the WRONG PROGRAM ID for the venue (`5jnapfr…`; the pools are owned by **`LanMV9sAd7wArD4vJFi2qDdfnVhFxYSUg6eADduJ3uj`**, which our own `analysis/coverage_expansion/RESEARCH.md` line 221 already had recorded as VERIFIED), so a correct offset would still have matched nothing. ⭐⭐ **`moonshot` added the same day: 409-byte pool, base **24**, quote mint NOWHERE in the struct so the venue is ONE-SIDED** - found by asking the indexer for pools BY VENUE (`/dexes/<dexId>/pools`, any age, keyless, paced), which is a better route than waiting for a venue to appear in the launch feed (moonshot was 0 of 113 new pools). ⛔⛔ **And a dexId is a BRAND, not a program: 20 of 20 pools under the `moonshot` label are METEORA DBC accounts**, so the venue behind a label is readable only from the pool's OWNER. ⭐ Moonshot's id in our map was RIGHT, LaunchLab's was wrong, both found by reading the pool. **13 venues queried now, 3 still not** (`obric`, `openbook`, `solfi`). ⛔⛔ **And raydium_clmm's third offset 454 is a REWARD-mint slot**, so a pool only counts if it HOLDS A VAULT OF OUR MINT; an unreadable vault set (Raydium v4's shared authority) is `pair_verified: None`, a third answer, never a pass | same, `analysis/pool_offsets/probe.py` |
| ⭐⭐ **THE FOURTH FIXTURE ROUTE WORKS, and a launchpad can only be measured at LAUNCH** | Three routes came back empty for the last five venues and the reason was in the ROUTES. ⛔ **A LaunchLab token MIGRATES**, so a mature mint's pair list only shows where it went: **USELESS** (letsbonk.fun, 64,262 holders, $15.5M 24h volume) resolves **30 pairs** and I resolved every one on chain - 14 meteora_dlmm, 7 damm_v2, 6 orca, 3 clmm, 2 cpmm, 1 v4, **ZERO LaunchLab**. ⭐⭐ **The launch feed is the route**: GeckoTerminal labels new pools, `raydium-launchlab` was **5 of 53** in one sample, and a pool minutes old has not migrated. ⭐ **Verified as output**: `poolstate.state()` with the indexer OFF derives the pool by `memcmp@205`, the same address the feed listed, reads **$184.12 of SOL** and returns `pool_live`. ⛔ **LaunchLab is the THIRD venue whose vaults the pool does not own** (authority `WLHv2UAZm6z4KyaaELi5pjdbJh6RESMva1Rnn8pJVVh`), so the old reader would have published $0 for it. ⭐⭐ **And 3 of the 4 fresh pools are quoted in TOKENISED EQUITIES - HOODx and METAx, both sharing SPYx's freeze authority** - so the asset-pairing mechanic is not StonkFun's alone; ⛔ our VALUED map prices only SOL/USDC/USDT, so those pools read **UNVALUED**, the STONK failure again, and ⚠️ **the map was NOT widened after seeing which rows failed** | `data/findings/POOL_DERIVATION_2026-09-23.md` §8-9, `docs/ASSET_PAIRED_TOKENS.md` |
| ⭐⭐ **the 30-pair cap is now BOUNDED: EMBER derives to 117 pools on chain** | Standing rule 18 says every all-pairs total at 30 pairs is a FLOOR and *"the remainder cannot be bounded"*. ⭐ **Derivation is not capped: EMBER returns 30 from the indexer and 117 from chain**, across meteora_damm_v2, meteora_dlmm, orca_whirlpool, pumpswap and raydium_clmm. ⚠️ Rule 18 still holds for the INDEXER; what changed is that we no longer have to stop there | `poolstate.state()` |
| ⛔ **`gone` is replaced by four measured states, and NOTHING is overwritten** | `pool_live` / `pool_emptied` / `curve_died` / `pool_closed` / `not_found` / `unreadable`, thresholds pre-committed. ⛔ **`pool_closed` is the ONLY state that means what `gone` claimed.** ⛔⛔ **`status` stays `gone` on every outcome row (rule 8); the measured state goes to a SIDECAR `data/pools/state.jsonl` keyed on the contract**, append-only, and readers prefer `pool_state` when present. ⚠️ **A backfill measures the pool TODAY**: a pool drained after we scored it is indistinguishable from one drained before, every row carries `backfilled: true`, and no rate over these rows may be called a rate at scoring time | `PRECOMMIT_pool_state.md` §3-4, `analysis/pool_offsets/backfill.py` |
| ⛔⛔ **AND THE `pool_closed` RULE WAS TOO LOOSE - CAUGHT BY ITS OWN BACKFILL AT ROW 25** | The pre-commit said `pool_closed` means the derived pool does not exist on chain. ⛔ **A PDA is a pure function of the mint, so we can derive a pump.fun curve for a token that NEVER LAUNCHED ON PUMP.FUN** - `BsskZM8NNi6ayj3h9KUwawMuuY8hU4iB1QYxEaSuJAfh` ("business") does not end in `pump`, its curve was never created, and its non-existence was scored as a closed pool. **An address that was never created is an ABSENT VENUE, not a closed pool** - the `authority_live=None` shape again. ⭐ **Corrected in `PRECOMMIT_pool_state.md` §3a, dated, not edited away: `pool_closed` now requires EVIDENCE THE POOL ONCE EXISTED**, which is the pool an outcome row actually priced (`known_pair`); a speculative PDA that does not exist yields `not_found`. ⚠️ **So `pool_closed` is rarer by construction and `not_found` now absorbs two different things**, both stated before the re-run. ⭐ The 33 rows measured under the old rule are kept at `data/pools/state_v1_superseded.jsonl` (rule 8) - **1 of the 33 was the wrong label**; `test_solpda.py` fails at the AST level if any branch other than the `known_pair_absent` one can assign `pool_closed` | `PRECOMMIT_pool_state.md` §3a |
| ⛔⛔ **TWO QUOTE-SIDE READS WERE PUBLISHING A NUMBER WE NEVER READ, both measured before the corrected run** | **(1) A pump.fun curve's quote side is NATIVE SOL in its own lamports**, which `getTokenAccountsByOwner` cannot see, so every curve read **$0**. ⭐ **Measured on 26 live curves: 2 hold >= $10 above the rent floor and one holds 1.978990 SOL = $226.61** - those would have been labelled `curve_died` with "$0". Now `lamports - rent_exempt(len)` at the run's SOL price, and **unknown if any input is missing, never 0**. **(2) A venue that owns no token accounts also read $0**: ⛔ **all 3 sampled Meteora DBC pools and the one FluxBeam pool return ZERO vaults**, and **12 of 120 recorded pairs are DBC**. ⭐ **`pooldiscovery.vaults_from_struct()` fixes it with NO layout assumed**: every 32-byte window of the pool's own data is a candidate pubkey and the chain says which are real SPL token accounts - a vault address is in the struct by definition. Measured: 2 vaults each for DBC (base + WSOL, $3.47 / $2.98 / $0.58) and 3 for FluxBeam ($14.66). ⛔ **And when neither route reads a vault, `quote_usd` is None with a reason, so such a pool can no longer produce `pool_emptied` or `curve_died`** - both of those assert there is no money in it | `PRECOMMIT_pool_state.md` §3b, `pooldiscovery.vaults_from_struct` |
| ⭐⭐ **THE RESULT: 120 `gone` contracts re-measured, and FOUR of them can be round-tripped for $100 TODAY** | Same population as the ladder run, 11 venues queried, 3,720 RPC calls / 0 errors / 0 rate-limited / 20.4 min, then the whole set re-measured under the corrected reader (both passes on disk, append-only). **`curve_died` 81 = 67.5% [58.69, 75.22]; `pool_emptied` 32 = 26.7% [19.57, 35.21]; `pool_live` 7 = 5.8% [2.85, 11.55]; `pool_closed` 0; `not_found` 0; `unreadable` 0.** ⭐⭐ **Every one of the 120 has a pool on chain, and the ladder's 15 `NO_POOL_FOUND` resolved 15 of 15** - the pre-registered prediction asked for 8 and named `curve_died` as dominant, and both hold. ⭐⭐ **4 of the 120 are round-trippable for $100 today: APEZCAT $91.94 back (8.06%), TRADER $92.52 (7.48%), CATP $92.24 (7.76%), PERPY $90.80 (9.20%)** - ⚠️ quotes, not fills, nothing signed. ⛔⛔ **CORRECTED the same day: only TWO of the four had the depth ALREADY IN THE POOL** (APEZCAT $1,238.65, TRADER $225.80); CATP ($11.45) and PERPY ($11.04) hold less than the probe, so their round trip is **SELF-FINANCED** - the sell leg is paid by the buy leg. ⭐ **The control is unambiguous: a BOUNCER mint whose curve holds $0.00 returns $92.22 on a $100 round trip.** So the honest headline is **2 of 120 with pre-existing $100 depth, 4 of 120 round-trippable**, and `analysis/pool_offsets/roundtrip_live.py` now carries `self_financed` on every row. ⛔ **3 of those 4 are visible ONLY because of today's two reader fixes**: TRADER and CATP hold their whole quote side as native SOL in a bonding curve, PERPY's only pool is a Meteora DBC whose vaults the old reader could not see. ⛔⛔ **And 3 of the 7 `pool_live` cannot be exited** (Robinhood NO_SELL_ROUTE on $22.67 of reserves, two TOTAL_LOSS), which is what keeps `pool_live` from meaning sellable | `data/findings/POOL_DERIVATION_2026-09-23.md` §5-6 |
| ⛔⛔ **the wrong-pool defect, measured properly - BOTH of my earlier numbers were wrong** | ⭐ **The recorded pair was a real pool of the mint 120 of 120 times**, so the ladder's *"not a discovered pool, 27 of 120 (22.5%)"* was the LADDER failing to surface the curve, not a bad record. ⛔ **The real defect: the recorded pair was NOT the pool holding the most quote side in 17 of 119 (14.3% [9.11, 21.69])** - and "3 of 3" was an anecdote from three contracts, never a rate. ⚠️ **I also nearly published this at 61 of 61 (100%)**: the first version compared a full-precision pool value against `quote_usd_max`, which is rounded to 6 dp on the way out, so every row failed an equality test. Caught before it left the terminal. ⛔ **`pool_closed` = 0 is now a meaningful zero, not the old tautology**: 86 of the 120 recorded pairs are bonding curves, which are never closed when a token bonds, so ⭐ **the non-tautological slice is the 34 real AMM pool accounts (12 DBC, 11 DAMM v2, 10 PumpSwap, 1 FluxBeam) and all 34 still exist**, n=34 | same, `data/pools/state.jsonl` |
| ⭐ **`journal.py`'s `gone` now says what it actually observed** | The word is **not renamed** - that relabels outcomes across the whole record and needs its own pre-commit - but the row no longer pretends to know why. Every `gone` row carries ⭐ **`status_reason`**: *two indexer lookups of the recorded pair returned nothing, this is OUR LOOKUP FAILING, 0 of 120 such contracts had a closed pool and 4 could be round-tripped for $100, the measured state is in `data/pools/state.jsonl`*. ⛔ **Asserted on OUTPUT: `test_gonelabel.py` calls `record_outcome` in the sandbox and reads the row back off disk** (12/12), because `record()` is a whitelist and this repo has lost five fields to compute-without-persist. A non-`gone` row carries `None`, never a leftover string | `journal.py`, `test_gonelabel.py` |
| ⛔ **the last five venues have NO offset, and three fixture routes all came back empty** | An offset is only measurable on a pool already known to belong to a mint. Tried today: **the indexer's pairs for 8 of the most liquid mints (240 pairs) → 0 owned by any of the five**; **Jupiter's launch feed → unusable, because `firstPool.id` is the MINT not a pool** (31 of 37 resolved to a Token program), and the one `raydium-launchlab` launch in the sample has **52 holders and not one program-owned vault**, so no pool exists for it yet; **the indexer's dexId search → launchlab, obric and openbook return 0 Solana pairs.** ⭐ **The structural reason now has evidence behind it: `openbook` is an order book whose accounts are MARKETS not vaulted pools; `obric` and `solfi` are PMMs, the class we already measured as $446.8M/day on 17 accounts launching nothing; `moonshot` and `raydium_launchlab` are launchpads whose fresh curves can never rank inside a liquid mint's 30-pair cap.** ⚠️ **A FLOOR, not completeness**: three routes failing means no fixture, not no pool. The cheap route that would work is a LaunchLab or Moonshot token that has actually TRADED | `data/findings/POOL_DERIVATION_2026-09-23.md` §8 |
| ⛔⛔ **the `BsskZM8NNi6a…` case, CORRECTED BY ITS OWN ROUND TRIP - I overstated it hours earlier in this session** | I wrote that it *"was never dead"*. **It is not sellable.** The chain read is right: `gone` → `pool_closed` → `not_found` → ⭐ **`pool_live`, $14.64 of WSOL in a FluxBeam pool** (`FLUXubRmkEi2q6K3Y9kBPg9248ggaZVsoSFhtJHSrm1X`, executable, absent from our AMM map entirely), and ⭐ **our recorded pair was the RIGHT pool all along - the bug was the READER, not the record.** ⛔⛔ **But a live $100 Jupiter round trip returns $0.0061, a 99.9939% cost: TOTAL_LOSS.** ⭐ **That is exactly why `pool_live` may never be read as sellable** - the state describes a pool, the exit is `chainfields.round_trip()` and only that. ⚠️ **And there are TWO contracts symbol'd `business` in this one 120-contract sample** (`BsskZM8NNi6a…` on FluxBeam, `4NjzeT37BXXa…` on Meteora DBC, also TOTAL_LOSS at $0.84) - standing rule 2 on schedule | `data/findings/POOL_DERIVATION_2026-09-23.md` §4 |
| ⛔ **Bubblemaps: LINK OUT, do not rebuild, and do not sign up** | Frank: *"There is already bubblemaps. Either reverse engineer or embed it."* ⭐ **Decision: AGGREGATE, link out.** Probed 2026-09-23: `https://v2.bubblemaps.io/solana/token/<MINT>` returns **200**; the iframe `https://iframe.bubblemaps.io/map?chain=solana&address=<MINT>&partnerId=demo` also returns 200. ⛔ **But their docs say `demo` is TEST-ONLY and production needs a dedicated `partnerId`, which is an issued credential tied to our domain - that is rule 9 (never sign up) and rule 17 (never take a dependency that needs auth), so it is FRANK'S CALL ALONE.** ⚠️ **Their pricing is NOT ESTABLISHED** - their quickstart states no figure and I am not guessing one. ⭐ The link ships now: zero auth, zero cost, and if they go down our facts still load. ⛔ Labelled as theirs and as UNVERIFIED by us | `docs/BUBBLEMAPS.md` |
| ⛔⛔ **`gone`, asked properly: the pool DERIVED from the mint on chain. MY PREDICTION WAS WRONG AND THE NUMBER BARELY MOVED** | I predicted 15-45%. Measured: ⛔ **2.5% [0.85, 7.09], 3 of 120** `gone` contracts have a pool holding >= $100 of quote side on chain — **and the 1.7% Frank rejected sits INSIDE that interval.** ⭐⭐ **The real correction is to the WORD, not the rate: 105 of 120 HAVE a pool, 87.5% [80.4, 92.3]; the dominant state is POOL_QUOTE_DUST, 96 of 120 — the pool EXISTS and has been EMPTIED.** ⛔ **My causal story was wrong too**: venue split `pumpfun_curve` **82** vs `pumpswap` **25**, so most never bonded and died on the curve, which is exactly when an empty pool is the honest answer. ⚠️ **A52 is narrower than stated**: our recorded pair WAS a discovered pool **93 of 120** times, so the wrong-pool defect is **27 of 120 (22.5%)** — ⭐ **but 2 of the 3 contracts with real money had a recorded pair that is NOT the pool holding it.** ⭐⭐ **Three `gone` contracts are verified sellable TODAY on a live Jupiter $100 round trip: NOOS $90.13 back, JEANB $90.24, LOOONGJAK $90.02.** ⚠️ Quotes, not fills | `data/findings/GONE_POOLS_2026-09-23.md`, `PRECOMMIT_pool_discovery.md` |
| ⭐⭐ **the $100 band: ZERO false positives over 21 contracts, and a MEASURED, BOUNDED floor** | The threshold was fixed before any data was read. It split sellable from unsellable on **all 6** non-dust sample rows, and on a second ALREADY-LABELLED set of 15 scored **86.7%** with ⭐ **0 false positives** — nothing it called >= $100 failed to sell. ⛔⛔ **It MISSED 2 of 15 sellable tokens** (STONKBROS $93.56, GO $96.44, both NO_POOL_FOUND). ⭐ **The floor is arithmetic: 113 of 120 rows were read to COMPLETION and every truncation landed on a NO_POOL_FOUND row (7).** ⛔ **I first published a ceiling of 8.3% and it was TOO LOW** - it forgot the 3 rows holding a vault in an asset we cannot price. Corrected: 7 truncated + 3 unvalued + ⭐ **0 from the AMM-map gap (MEASURED: the one unknown program on 10 rows owns an account holding 31,487 token accounts across 31,479 DISTINCT MINTS plus 233.96 WSOL - a router, not a pool, and adding it to `NOT_POOLS` changes no verdict)** = 10 rows, so the honest answer is **at least 2.5%, at most 10.8%**. Still short of my predicted 15%. ⭐⭐ **Run twice 14 minutes apart: 0 verdict changes across 120 contracts, every figure identical** ⛔⛔ **EXISTENCE IS NOT LIQUIDITY**: a pool holding **$41.07** of WSOL returns **NO_SELL_ROUTE**, so `POOL_QUOTE_10` may NEVER be read as sellable, and discovered reserves are shape, never an exit price. ⭐ **`excluded_owners` (rule 15) diagnosed both misses, and the causes were NOT what I first wrote**: ⛔⛔ **STONKBROS' quote asset is STONK, which the pre-committed VALUED map does not price - and `docs/ASSET_PAIRED_TOKENS.md` ALREADY RECORDED that valuing only SOL/USDC/USDT reads it as $0.03 while Jupiter sells $100 of it for $94**, so a perfectly found pool would still have graded UNVALUED. **The same failure the repo had already documented, reappearing in a new module.** GO's cause is different: a Token-2022 mint with 2,884 token accounts whose pool vault is not in the top 100 by base amount. ⚠️ **And I printed those owner addresses truncated to 14 chars, then completed them from memory in a follow-up query, which returned two nonexistent accounts - a truncated identifier is not an identifier.** ⛔ **Neither fix was applied before the run, deliberately** — editing the map after seeing which rows failed is post-hoc tuning | same, `analysis/gone_pools/threshold_validation.json` |
...
| ⛔⛔ **I OVER-ALARMED ON THE RWA QUOTE LEGS AND FRANK CAUGHT IT** | `pair_legs` first called an issuer-controlled quote leg a **DANGER verdict** (`VERDICT: ISSUER CONTROLLED LEG`). Frank: *"rwa stonk pairs seem to be safe... there may be a reason nobody checks the other leg."* ⭐ **He is right and there is a reason: a freeze authority and a `permanentDelegate` are exactly what a regulated tokenised security is REQUIRED to carry** so the issuer can comply with a court order, a sanctions listing or a securities rule. **A tokenised equity WITHOUT them would be the anomaly.** ⛔ **So it is a DISCLOSURE, not a verdict**: the field is `issuer_powers_disclosed`, the line is `disclosure`, and the wording says the powers are expected. ⭐ **What stays a genuine flag is the TRANSFER TAX**, its own `taxed_legs` field, because it is charged on the way in and again on the way out and is a round-trip cost rather than a yield: **VCF 800 bps, Circuit 400, ZCAT / RAYCAT / PURR-sol / LOOP / KNOTS 300 each** - and KNOTS, LOOP and STONKCAT still carry a LIVE `transferFeeConfigAuthority`, so the rate can be changed after you buy. ⭐ **Issuer terms now RESEARCHED** - see the two rows below | Frank, via the relay; `docs/SITE_API.md` §9, BACKLOG D10 |
| ⭐⭐ **THE RWA ISSUER RESEARCH, done: the two quote legs are DIFFERENT INSTRUMENTS, and our own alarm was partly FALSE** | Read from the mint accounts and the issuers' own files, 2026-09-23. **SPYx = "SP500 xStock", issuer Backed Assets (JE) Limited**; **COPX = "Global X Copper Miners ETF - Backpack Securities", issuer Backpack Securities** (not Backed - that corrects our own note). ⛔⛔ **SPYx is a TRACKER CERTIFICATE, not the share: no voting, dividends by rebase, and it cannot move to a brokerage account. COPX is a tokenised share REDEEMABLE 1:1 for the real one via ACATS, with cash dividends.** ⛔ **Backed's own geoblock excludes US citizens, US residents and anyone otherwise taxable in the US, and says the securities are unregistered under US law**; `docs.xstocks.fi`: *"xStocks are not intended for distribution in the United States, to any US person"*. ⚠️ **Backpack's position is NOT ESTABLISHED** - its agreement is PDFs we did not read, and Backed's own terms page **geoblocks us**, so the structure rests on Kraken's distributor FAQ plus press, tagged `from_press` and never as ours. ⭐⭐ **The dividend mechanism is visible ON CHAIN: SPYx's `scaledUiAmountConfig` multiplier is 1.003909240011759 rising to 1.005714560286254; COPX's is exactly 1.** A rebasing instrument and a cash-dividend one are distinguishable from the mint alone. ⛔ **And neither has a transfer fee** - the assets that actually cost you money on a round trip (VCF 800 bps, Circuit 400, ZCAT/RAYCAT/PURR-sol/LOOP/KNOTS 300) carry none of the alarming readings. **The alarming reading and the real cost are on different legs** | `data/findings/RWA_LEGS_2026-09-23.md`, `legs.ISSUERS` |
| ⛔⛔ **we were saying something FALSE about two mints, and it was the cry-wolf direction** | `transferHook` is present on SPYx and COPX with **`programId: null`** - the slot is EMPTY and no issuer code runs on any transfer - and we published *"every transfer runs issuer code that can reject it"* about both. ⚠️ **That is the `authority_live=None` error pointed the other way: not-happening rendered as happening**, and it is the worse direction, because a false alarm is what makes a real one ignorable. ⭐ **Fixed by generating every sentence from the extension's STATE, not its presence** (`legs.power_sentence`): an explicit null hook says the slot is empty, a MISSING key still reports the capability, `defaultAccountState: initialized` says accounts are not frozen today, `paused: true` says PAUSED RIGHT NOW, and a `permanentDelegate` whose address could not be read **still reports the power and says who holds it is unknown** - the first version returned silence there, which is the same not-checked-reads-as-clean bug, and the suite caught it within a minute. ⛔ **`intel._leg_authorities` was a SECOND COPY of `legs.read_mint` and the two had already drifted; there is one reader now** | same |
...
| ⭐⭐ **the Helius POOL receiver is LIVE with real rows** | Webhook **`d8765cda-6cc4-443e-870b-a004df20af3e`** → `https://rxofejxostyqlgjlzqmk.supabase.co/functions/v1/helius-events` → `public.chain_events`. ⭐ **First inbound row 2026-09-23 03:53:13Z; 1,571 rows by 04:06:40Z.** Runs on Supabase, NOT Frank's PC. Auth verified in both directions: no-header, wrong secret and `Bearer <secret>` all 401, exact secret stores. ⛔ **Nothing alerts yet and no page reads it, so it is IN PROGRESS, not SHIPPED** | `docs/CHAIN_EVENTS.md`, `heliushook.py` |
| ⛔⛔ **webhook cost, measured: the free tier affords ONE busy pool** | 18 pool addresses delivered **5.55 events/sec = 14,574,000 credits/month** against a **1M free tier**; 14 addresses still ran ~1.6M. ⭐ **And liquidity does NOT predict volume - it is inverted: STONK at $5.1M fired ONCE in 88s, JEANPHIL at $323k fired 389 times**, a 389x spread the wrong way. ⛔ **Type filtering does not rescue it**: on 1,571 delivered rows `SWAP` is only **9.65%**, while `UNKNOWN` is **47.82%** and `TRANSFER` **41.38%** - and that 89% is exactly where a Meteora liquidity op hides. ⭐ **The affordable drain detector is POLLING reserves**: one `getMultipleAccounts` per 30s covers ~150 pools for **87,600 credits/month** and measures reserves falling rather than a type we distrust | same §3 |
| ⛔⛔ **the STORAGE budget binds harder than the credit budget** | The free Supabase database is **500 MB TOTAL**, shared with everything already in the project. At the 18 addresses I registered, `chain_events` writes **479,520 rows/day**, which at ~2 KB of raw payload each is ⛔ **982 MB per DAY - the whole database in under twelve hours**. Even the 4 budget-passing addresses fill it in a month at full payload. ⭐ **Fixed in receiver v6: full payload ONLY for `POOL_CREATE` / `LIQUIDITY_ADD` / `LIQUIDITY_REMOVE` / `LARGE_TRANSFER`; `SWAP` and `OTHER` keep every small field plus a bounded 20-per-hour raw sample.** ⛔ **An omitted payload is an explicit `omitted: true` marker, never a null** (rule 5). ⚠️ **The 2 KB row size is an ESTIMATE, not a measurement** - the database went down before it could be measured; re-measure with `pg_total_relation_size` before quoting capacity | `docs/CHAIN_EVENTS.md` §3 |
| ⛔⛔ **the receiver's own 5xx WAS the outage, and two more bugs fell out of testing the fix** | Frank: *"That is a burst problem, not a capacity problem. Fix it with connection pooling and backpressure on our side, not a bigger plan."* ⛔⛔ **The amplifier was one line of ours:** the insert path answered **500** on 401/403/5xx so "the event is not lost", and **Helius retries a non-2xx**, so our answer to a struggling database was to ask for the same write again. ⭐ **Rule now, pre-committed in `backpressure.ts`: this receiver may NEVER answer 5xx because a write failed** — it absorbs the failure, logs a `chain_events_drop` line with a count and a signature, returns 200. Congestion gets ONE jittered retry; privilege and payload get none. A breaker opens after 3 consecutive congestion failures for 30s and attempts no insert while open — ⚠️ **per-isolate, so a damper, not a guarantee.** ⭐ **Burst measured: 248 batches / 2,480 rows, 200 on all 248, 11/11 control reads OK, 0 rows created, 75 rows/s.** ⚠️ **But it achieved 7.5 POST/s against the ~11 req/s that caused the outage, so "a burst cannot exhaust the pool" is NOT established**, and the breaker has never been seen opening under real congestion. ⭐ **What is: cost per EVENT fell from 2 PostgREST requests to 0.1, a 20x reduction.** ⛔⛔ **And the burst found LIVE DATA LOSS: `resolution=ignore-duplicates` never ignored anything** (PostgREST conflicts on the PRIMARY KEY, ours is a unique index on `signature`), so all 248 returned 409 — **and a POST of an array is ONE statement, so one already-stored signature killed every genuinely new event in the batch.** Fixed with `?on_conflict=signature`. ⛔ **`stored` was also `rows.length`, the number SENT** — a replay that wrote nothing said `stored: 3`; it now reads PostgREST's `Content-Range`, or **null with a reason** | `docs/CHAIN_EVENTS.md` §4, §4a; BACKLOG C39, A65 |
| ⛔⛔ **"watching: 20" was not a fact about the system, and DDL now runs here** | Three lists answer "what are we watching" and only one decides delivery. Measured 2026-09-23 14:45Z: **mirror 20, git 6, Helius 6.** `chain_watch_sync` ignored the payload's `active` and hard-coded `active = true` on conflict, so the mirror could drift up and never down — **and my first fix silently RE-ACTIVATED the eight I meant to retire**, which is how I found it. Fixed by migration; **deactivated, never deleted** (6 active / 20 inactive). ⭐ `heliushook.health()` now compares git / **Helius `accountAddresses`, the delivery list** / mirror every run and names each difference, with `agree: None` when a list is unreadable. ⚠️ **CLAUDE.md's "nothing on disk can run DDL" is still true of the DISK but no longer of this session** — `apply_migration` over the Supabase MCP works, which is what BACKLOG A41 was blocked on | same, BACKLOG A66 |
| ⭐ **Helius DOES type a PumpSwap drain correctly** | **3 of 3** of our known drains came back `WITHDRAW`: USDCAT 09-21 19:54:19Z, OWL 09-22 07:14:06Z, X7 09-22 10:27:06Z. ⚠️ **But TSLA is dead too and its pool shows no `WITHDRAW` in 100 txs** (SWAP 14, UNKNOWN 43, TRANSFER 43) and UOTF's pool is delisted, so the typed path catches **3 of 5**, n=5. **An observation, not a rate** | same §2 |
| ⛔⛔ **I took out the shared Supabase REST API, 2026-09-23 ~04:06Z** | The receiver read `chain_watch` on **every** inbound event; at 5.5 events/sec that is 11 PostgREST requests/sec on a free-tier project, and every 500 it returned made Helius retry and multiply it. **PostgREST stopped answering for EVERY table including pre-existing ones**, then the database stopped accepting new connections. `/rest/v1/` still answered 401 in 0.29s because it never touches the DB. ⭐ **The site was unaffected - it reads `data/` from git**, and only `site/api/helius.mjs` (watching 0 addresses) uses Supabase; `supa.publish` swallows the error and returns `(False, reason)`. Fixed with a 60s instance cache that returns **null**, never a stale list | same §4 |
| ⭐ **Fomo net-positive share: use 5 to 7 percent, multiply-sourced** | Frank, 2026-09-23: *"Use 5 to 7 percent net positive, multiply-sourced. Do not quote the 500 figure as fact."* ⛔ **The viral "only ~500 traders ever above $1,000" is ONE X post and is not to be repeated as fact.** Three independent derivations land in the band: **5.69%** (27,130 / 476,627 wallets, 09-02), **6.16%** (DWF, ~292,000 wallets, realized, 90d), **6.0% [5.8, 6.3]** (ours, 45,853 closed positions at >= 2x). ⚠️ All three fold break-even into losses, and only **229** Robinhood Chain FOMO users are above $10,000 | `docs/CHAINS.md` §2.6 |
| ⛔⛔ **the funding graph is already defeated, so sybil detection needs STACKED signals** | Frank, 2026-09-23: sophisticated bundlers *"fund 20-plus wallets through paths that defeat naive funding-graph clustering."* ⭐ **Four signals, ranked by how hard they are to defeat: same-block co-buying** (splitting a buy is what makes them arrive together), **wallet age and history** (ageing 20 wallets costs time and rent), **post-launch behavioural correlation** (the exit is harder to disguise than the entry), and shared funder — ⛔ **which is now kept only for hub identification.** ⛔ **Output is a published CONFIDENCE LEVEL, never a binary verdict.** ⚠️ **NOT STARTED** - scoped only | `docs/CONCENTRATION.md` §5a |
| ⛔⛔ **the stacked-signal sybil detector: BUILT, RUNNING UNATTENDED, and TWO of its four signals were KILLED BY THEIR OWN CONTROLS the day it was written** | `sybil.py` under `PRECOMMIT_sybil_v1.md`, written before a single wallet was read. ⛔⛔ **S1 same-slot co-buying is DEAD: it fired on 9 of 9 ordinary pump.fun graduations**, co-buy share 0.625-0.933, median 0.750, because **a Solana slot is ~400 ms and packs many transactions, so sharing one is the BASE RATE at a launch.** Weight is **0**, per the pre-commit's own section 7, not re-tuned. ⚠️ **The one STRONG verdict it produced (`55Tn9f8Njei…`) is WITHDRAWN - never quote it.** ⛔⛔ **S3 exit correlation measured nothing: `exit_share` 1.000 on all nine, including one-seller mints**, because `flows()` reads a mint's first ~300 signatures and every sell therefore sits inside the 600s window BY CONSTRUCTION - **standing rule 13, in code I wrote that morning.** It now returns `fired: None` / `unevaluable: true`, and **unevaluable is not a clean negative.** ⭐⭐ **The thesis survived: on the one mint taken deep, S4 shared-funder found NOTHING (largest group 1 of 10) and S2 did not fire (2 fresh of 12 datable) - a funding-graph detector, which is what every public bundle checker is, would have returned "clean".** ⛔ **So v1 has no signal both alive and demonstrated to discriminate, and that is the honest headline.** Output is a confidence level with the signals named, never a binary. ⭐ The cluster-buy half shares the same machinery (registry 43 recurring wallets of 474 over 60 mints) and **says in its own response that it is never a buy signal** | `PRECOMMIT_sybil_v1.md`, BACKLOG B10 |
| ⭐⭐ **holder RECURRENCE, measured 2026-09-23** | Over **60 consecutive graduations**: **474** distinct top-10 wallets, and ⭐ **43 of them (9.1% [6.8, 12.0]) are top-10 holders of MORE THAN ONE mint**. ⭐ **Three wallets top-hold 10 of the 60 (17% of all launches in the window)**. ⭐ **Median token has 20% of its top 10 recurring**; one fresh graduate measured **80%**. ⛔ So "8 wallets hold 64%" usually means industrial participants present launch after launch, NOT one buyer cornering supply. ⚠️ Always a FLOOR: it proves presence, never absence | `docs/CONCENTRATION.md`, `analysis/holder_recurrence/` |
| ⛔⛔ **the sybil-cluster framing FAILED on this population, and that is the finding** | Top holders of fresh pump.fun graduates are **not** fresh wallets: every one carries **3,000+ signatures**. ⛔ **0 fresh wallets in the top 10 of either of two tested graduates.** So collapsing wallets by shared funder would have manufactured clusters. A wallet is only collapsible when its whole history was reached AND it holds <= 250 signatures. ⭐ **"All top holders are established traders" is a real negative result and the module says it out loud** | same |
| ⛔⛔ **WE ARE NOT SELLING A SCORE** | Frank: *"We need to seriously improve our scoring system before we can sell it."* ⭐ **Retire it as a product rather than improve it** - Marino has not moved. ⭐ **The sellable thing is checkable FACTS nobody publishes**: bundled or not, liquidity real or phantom, exit depth at a stated size, authorities live or revoked, who else holds it. **None needs a hit rate to defend because none predicts anything.** ⛔ `test_intel.py` fails at the AST level if any API field name contains `score`, `grade`, `rank`, `expected_return`, `prediction` or `rating`, and if a model client is imported into `intel.py` | `docs/SITE_API.md` §6 |
| ⚠️ **the n=4,392 score experiment** | ⛔ **NOT in this repo and NOT re-derivable by this session.** A brief reported score inverting among early 2x runners (5% vs 26%). I could not find it in the tree or in any commit, so **it is not quoted as ours**. ⭐ The conclusion it supports was already this repo's standing position (Marino, `dashboard.py` never displays score, `test_scoreband.py`), so the action is unchanged either way | standing rule 1 |
| ⭐ **architecture, settled 2026-09-23** | **We will not write custody code.** Pools are created on **Raydium or Meteora**, swaps route through **Jupiter**, and any program of ours is a thin deploy-and-configure layer only | Frank, via the relay |
| ⭐⭐ **the wedge, and it is measurable** | **"connect your wallet and find out what you actually own."** On Frank's own PURR position of **19,772.37 tokens his tracker showed $163.92; a live sell quote for that exact quantity returns $158.50.** Neither is wrong, only one is what he receives. ⭐ `intel.wallet()` answers the four questions no portfolio tracker asks: **is there a market at all, is the cap backed, what do YOU get for selling what YOU hold, and is it taxed** | `docs/SITE_API.md` |
| ⛔ **wallet connection: yes, read-only, and enforced in code** | The site asks the wallet for its **public key only**. ⛔ **No signature request, no transaction, no approval, no delegate, no private key - and a message-signature login is still a signature request and is out of scope.** ⭐ **`test_intel.py` parses `intel.py` at the AST level and FAILS if any name matches `sign`, `send_transaction`, `keypair`, `secret_key`, `approve`, `delegate`, `set_authority`, `place_order` and a dozen more** | same |
| ⭐ **`resolve(ticker)` measured on the case that cost a day** | `resolve('EMBER', since='2026-09-10', chain='solana')` → **RESOLVED, dominance 538.95x**, winner `5dvXTZ5q…`. ⛔ **SIX different Solana mints are named "embercurve"**, and one of them (`EPT3ta6E…`) holds **$1.45M** and is excluded ONLY by the date rule. Without `since` the answer is **AMBIGUOUS and nothing is called real** | same |
| ⭐⭐ **where Frank can place a stop loss, verified on chain** | **Jupiter Limit Order V2, program `j1o2qRpjcyUwEvwtcfhEQefh773ZgjxcVRry7LDqg5X`** - executable, **195,276 program accounts**, newest activity **2026-09-23T01:17:56Z**. ⭐ **Authority model measured, not assumed: one maker signature at creation escrows the tokens; the KEEPER `j1opmdubY84LUeidrPCsSGskTCYmeJVzds1UWm6nngb` signs every `FillOrder` (25/25 txs have exactly one signer); 20 distinct wallets signed `CancelOrder`, so the maker can always cancel.** Cost **0.10%** on non-stable pairs + **0-0.5%** Ultra routing + **0.00225 SOL (~$0.27) refundable rent**. Stop loss, trailing stop (launched 2026-07-03, trail 0.5-90%), OCO, OTOCO. ⛔ **Tokens must leave the Ledger to be protected** - there is no stop that works on cold storage. ⛔⛔ **Jupiter's own docs CONTRADICT each other on transfer-fee Token-2022, which is PURR/ZCAT/KNOTS/LOOP/STONKCAT**; my chain sample found **0 of 20** Token-2022 mints in recent V2 orders carrying a `transferFeeConfig`, which is consistent with exclusion but n=20. **Only a real order settles it.** ⛔ Legacy V1 `jupoNjAxXgZ4rjzxzPMP4oxduvQsQtZzyknqvzYNrNu` is sunset - I measured 56 `CancelOrder` to 1 `InitializeOrder` across 2026. ⛔ The *developer* Trigger API is **Privy-custodial with JWT auth** and fails rules 9 and 17 | `docs/SOLANA_STOPS.md` |
| ⛔⛔ **a stop loss does NOT cover the dominant risk** | 3 of our confirmed deaths were **one `Withdraw` as the last event on the pool** (USDCAT, OWL, X7). Jupiter itself: *"Stop Loss is not guaranteed to execute... especially common with low-liquidity tokens such as new memecoins or tokens that have been rugged."* ⭐ **There is no price between healthy and zero for a keeper to sell into.** Position size is the control, not the stop | same |
...
19. ⛔⛔ **NOTHING PUBLIC UNLESS IT IS DERIVED ON CHAIN OR INDEPENDENTLY SOURCED**
    (Frank, 2026-09-23: *"We have to be sure we have our facts and receipts
    correct. Our data cannot be inaccurate."*). This is a hard gate on the public
    product: a fact card, the ledger, the whitepaper, any page.
    ⛔ **And specifically: NONE of our own collector statistics go into the
    whitepaper or any public document while the pipeline is still being
    corrected.** Frank caught that himself. The reason is on the record - `gone`
    was wrong about mechanism in 120 of 120 cases, the quote side of a bonding
    curve read $0 while one held $226.61, and two venues' vaults were unreadable
    so their pools read empty. ⭐ **What may be published is what a reader can
    re-derive**: an address, a pool, a vault balance, a live quote, with its
    provenance and its n.
...
| ⭐⭐ `intel.py` | **THE BACKEND THE SITE CALLS**: `liquidity` (all pairs, never one pool), `resolve` (every mint wearing a ticker, impersonators named), `phantom`, `exit_depth` (at the size actually held), `safety` (check.py published), `bundle_check` (⛔ UNVALIDATED, labelled), `paired`, ⭐⭐ `pair_legs` (**BOTH LEGS: what the issuer of the asset you are PAID in can do - a DISCLOSURE, never a danger verdict**), ⭐⭐ `wallet(pubkey)` READ-ONLY. Every response carries `provenance` and `not_checked` | ✅ **built and tested 2026-09-23, 66/66 in `test_intel.py`**; called live against EMBER, Hypurr, a public wallet and ⭐ **STONK over HTTP: `VERDICT: ISSUER CONTROLLED LEG`, SPYx flagged, 9 legs read**. ⭐ `test_intel.py` now FAILS if an endpoint exists and `intelserve.ROUTES` serves it not. ⛔ **NOT wired to the site yet** - that is the site session's work and the contract is `docs/SITE_API.md` |
| ⭐ `intelserve.py` | stdlib HTTP in front of `intel.py`, GET only, CORS open, `Cache-Control` from the per-endpoint TTL | ✅ **runs**: `python intelserve.py 8799`. ⛔ **POST/PUT/PATCH/DELETE/HEAD all return 405 before routing** - verified live. No auth, no secrets, no `.env` read |
| `site/api/helius.mjs` on Vercel | Helius webhook receiver → Supabase → Discord | ✅ **deployed and answering**, but watching 0 addresses |
| ⭐⭐ `sybil.py` | the stacked-signal coordination detector: 4 shallow + 1 deep mint a pass, from the **graduations** stage | 🟡 **runs UNATTENDED** - `data/liveness.json` → `sybil.analysed`, **8 rows, `last_origin: scheduled`, 2026-09-23 14:18:48Z**. ⛔ **No page and no message reads it, so it is IN PROGRESS, not SHIPPED.** ⛔ Two of its four signals are dead or unevaluable after their own controls (row above) |
| ⭐⭐ `crossingalert.py` | ALERT / SILENT_THIN / SILENT_UNMEASURED on every $1M / $5M crossing, hung off `milestones.check_outcome`'s `on_milestone` callback so the decision happens in the call that measured the depth | 🟡 **runs UNATTENDED** - `crossing.decisions`, **97 rows, `last_origin: scheduled`**. ⛔ Nothing pings yet |
| ⭐⭐ `legs.py` | the quote-asset registry → `data/legs/quote_assets.json`, from the **market** stage | 🟡 **runs UNATTENDED** - `legs.registry`, **130 rows, `last_origin: scheduled`**; 32 quote assets, 2 issuer-controlled (COPX, SPYx), 7 freezable, 7 taxed |
...
| ⭐⭐ **THE SITE BACKEND: nine read-only endpoints incl. ⭐⭐ `pair_legs` (BOTH LEGS), the wallet-connection boundary, the deployment options** | `docs/SITE_API.md` |
| ⭐⭐ **who actually holds it: pool vaults out, sybils collapsed, and the recurrence base rate** | `docs/CONCENTRATION.md` |
| ⛔⛔ **the stacked-signal sybil detector, and the two signals its own controls killed** | `PRECOMMIT_sybil_v1.md` |
| ⛔⛔ **is a contract we call `gone` actually dead? DERIVE the pool from the mint on chain** | `PRECOMMIT_pool_discovery.md`, `data/findings/GONE_POOLS_2026-09-23.md` (⛔ `PRECOMMIT_gone_onchain.md` is the superseded tautology) |
| ⛔⛔ **pool discovery rebuilt on DERIVATION: the measured offsets, the three corrections it forced on itself, and what it still cannot reach** | `data/findings/POOL_DERIVATION_2026-09-23.md`, `PRECOMMIT_pool_state.md` (§3a and §3b are dated amendments to the rule, not edits) |
| ⭐⭐ **live chain events: the Helius receiver, what each event triggers, and why the free tier cannot afford it** | `docs/CHAIN_EVENTS.md` |
| ⭐⭐ **stop losses and limit orders on Solana spot: what signs what, and why a stop cannot save him** | `docs/SOLANA_STOPS.md` |
| ⭐⭐ **asset-paired tokens: the verified mechanic, and the copper correction** | `docs/ASSET_PAIRED_TOKENS.md` |
| ⭐⭐ **the RWA quote legs: who the issuers are, what the instruments actually are, and the two sentences of ours that were false** | `data/findings/RWA_LEGS_2026-09-23.md` |
...
⭐ `heliushook.py` (**the Helius webhook control plane: list, register, sync, rate; `health()` compares git / Helius / mirror every run; never prints a key**) · ⭐⭐ `concentration.py` (**holders with pool vaults out, sybil clusters, and the recurrence registry**) · ⭐⭐ `sybil.py` (**the stacked-signal coordination detector under `PRECOMMIT_sybil_v1.md`: a confidence LEVEL with the signals named, never a binary; ⛔ S1 weight 0 and S3 unevaluable after their own controls; also `cluster_buys()`, which is explicitly NOT a buy signal**) · ⭐⭐ `intel.py` (**the site backend: nine read-only endpoints, provenance and honest nulls on every response**) · `intelserve.py` (GET-only HTTP) · ⭐ `allpairs.py` (⛔ **the all-pairs read: every pool for a mint, summed; `pair_count`, phantom detection**) · ⭐⭐ `poolstate.py` (⛔⛔ **THE PRIMARY PATH: pools DERIVED from the mint - a pump.fun curve PDA plus a `getProgramAccounts` memcmp at each venue's MEASURED offset - then the six pre-committed states. A pool only counts if it HOLDS A VAULT OF THE MINT, which throws out reward-mint slots**) · ⭐ `solpda.py` (**base58 both ways, ed25519 off-curve check, `find_program_address`; verified against addresses found by an unrelated route**) · ⭐⭐ `pooldiscovery.py` (⛔ **the token-account LADDER, now a SUPPLEMENT, no indexer and no AMM layout parsed: enumerate the mint's SPL token accounts, resolve each owner's program, a known-AMM owner is a pool, then its own vaults give the quote side. A LADDER, so a negative is a FLOOR and says so; `excluded_owners` records every owner it refused and why**) · ⭐⭐ `crossingalert.py` (**the crossing lane: ALERT / SILENT_THIN / SILENT_UNMEASURED per `PRECOMMIT_crossing_alert.md`; the silence is the feature**) · ⭐⭐ `legs.py` (**the quote-asset registry and THE ONE MINT READER: one RPC per quote mint, sentences generated from each extension's STATE not its presence, the researched issuer registry keyed on the MINT, published to `data/legs/quote_assets.json`**) · ⭐ `graduations.py` (graduation ledger) · `volintegrity.py` (volume M1/M2 under
...
python intel.py wallet <PUBKEY>                            # ⭐ what a wallet actually holds
python intel.py resolve <TICKER>                           # every mint wearing a ticker
python test_backpressure.py                                # ⭐ RUNS the receiver's load rule, in node
python poolstate.py <MINT>                                 # ⭐⭐ DERIVE the pools and say what state they are in
python poolstate.py                                        # ⭐ list which venues have a MEASURED offset
python pooldiscovery.py <MINT>                             # the token-account LADDER, now a supplement
python test_pooldiscovery.py                               # ⭐ 44 checks; its intel block was REWRITTEN when the chain became primary
python test_solpda.py                                      # ⭐ 58 checks: PDAs, the six states, and the §3a/§3b guards
python analysis/pool_offsets/probe.py                      # ⭐⭐ MEASURE each venue's mint offset -> data/pools/offsets.json
python analysis/pool_offsets/backfill.py --limit 120       # ⭐ replace `gone` with the measured state, append-only sidecar

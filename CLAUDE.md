...
| ⭐⭐ `intel.py` | **THE BACKEND THE SITE CALLS**: `liquidity` (all pairs, never one pool), `resolve` (every mint wearing a ticker, impersonators named), `phantom`, `exit_depth` (at the size actually held), `safety` (check.py published), `bundle_check` (⛔ UNVALIDATED, labelled), `paired`, ⭐⭐ `wallet(pubkey)` READ-ONLY. Every response carries `provenance` and `not_checked` | ✅ **built and tested 2026-09-23, 34/34 in `test_intel.py`**, called live against EMBER, Hypurr and a public wallet (4.8s for 10 positions). ⛔ **NOT wired to the site yet** - that is the site session's work and the contract is `docs/SITE_API.md` |
...
| ⭐⭐ **THE SITE BACKEND: eight read-only endpoints, the wallet-connection boundary, the deployment options** | `docs/SITE_API.md` |
...
⭐ `heliushook.py` (**the Helius webhook control plane: list, register, sync, rate; never prints a key**) · ⭐⭐ `concentration.py` (**holders with pool vaults out, sybil clusters, and the recurrence registry**) · ⭐⭐ `intel.py` (**the site backend: eight read-only endpoints, provenance and honest nulls on every response**) · `intelserve.py` (GET-only HTTP) · ⭐ `allpairs.py` (⛔ **the all-pairs read: every pool for a mint, summed; `pair_count`, phantom detection**) · ⭐ `graduations.py` (graduation ledger) · `volintegrity.py` (volume M1/M2 under

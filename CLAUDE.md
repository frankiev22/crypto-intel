...
| ⭐⭐ `intel.py` | **THE BACKEND THE SITE CALLS**: `liquidity` (all pairs, never one pool), `resolve` (every mint wearing a ticker, impersonators named), `phantom`, `exit_depth` (at the size actually held), `safety` (check.py published), `bundle_check` (⛔ UNVALIDATED, labelled), `paired`, ⭐⭐ `pair_legs` (**BOTH LEGS: what the issuer of the asset you are PAID in can do**), ⭐⭐ `wallet(pubkey)` READ-ONLY. Every response carries `provenance` and `not_checked` | ✅ **built and tested 2026-09-23, 66/66 in `test_intel.py`**; called live against EMBER, Hypurr, a public wallet and ⭐ **STONK over HTTP: `VERDICT: ISSUER CONTROLLED LEG`, SPYx flagged, 9 legs read**. ⭐ `test_intel.py` now FAILS if an endpoint exists and `intelserve.ROUTES` serves it not. ⛔ **NOT wired to the site yet** - that is the site session's work and the contract is `docs/SITE_API.md` |

...
⭐⭐ `intel.py` (**the site backend: eight read-only endpoints, provenance and honest nulls on every response**) · `intelserve.py` (GET-only HTTP) · ⭐ `allpairs.py` (⛔ **the all-pairs read: every pool for a mint, summed; `pair_count`, phantom detection**) · ⭐ `graduations.py` (graduation ledger) · `volintegrity.py` (volume M1/M2 under

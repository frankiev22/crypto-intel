...
| ⭐⭐ **THE RESULT: 120 `gone` contracts re-measured, and FOUR of them can be round-tripped for $100 TODAY** | Same population as the ladder run, 11 venues queried, 3,720 RPC calls / 0 errors / 0 rate-limited / 20.4 min, then the whole set re-measured under the corrected reader (both passes on disk, append-only). **`curve_died` 81 = 67.5% [58.69, 75.22]; `pool_emptied` 32 = 26.7% [19.57, 35.21]; `pool_live` 7 = 5.8% [2.85, 11.55]; `pool_closed` 0; `not_found` 0; `unreadable` 0.** ⭐⭐ **Every one of the 120 has a pool on chain, and the ladder's 15 `NO_POOL_FOUND` resolved 15 of 15** - the pre-registered prediction asked for 8 and named `curve_died` as dominant, and both hold. ⭐⭐ **VERIFIED SELLABLE on a live $100 Jupiter round trip: APEZCAT $91.94 back (8.06%), TRADER $92.52 (7.48%), CATP $92.24 (7.76%), PERPY $90.80 (9.20%)** - ⚠️ quotes, not fills, nothing signed. ⛔ **3 of those 4 are visible ONLY because of today's two reader fixes**: TRADER and CATP hold their whole quote side as native SOL in a bonding curve, PERPY's only pool is a Meteora DBC whose vaults the old reader could not see. ⛔⛔ **And 3 of the 7 `pool_live` cannot be exited** (Robinhood NO_SELL_ROUTE on $22.67 of reserves, two TOTAL_LOSS), which is what keeps `pool_live` from meaning sellable | `data/findings/POOL_DERIVATION_2026-09-23.md` §5-6 |
